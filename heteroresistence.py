...
def running(file_name, ignore):
    ...
    """
    start = time()
    file = nawkProcess(file_name)
    file['Raw'].replace('\n', float('NaN'), inplace=True)
    file.dropna(subset=['Raw'], inplace=True)
    df = file.apply(lambda df: mappingData(df['Gen-Position'],
                                            df['Raw'],
                                            df['Probe'],
                                            df['Position'],
                                            ignore.loc[ignore['Gen-Position'] == df['Gen-Position']]),
                                            axis=1)
    print(f'Total time to {file_name[:-4]} process:  {time() - start} seconds')
    return df


def nawkProcess(file_name):
...
def mappingData(gen, reads, lead, position, ignore):
    ...
    df['Ends'] = df['Read'].apply(lambda read: findNear(lead, read, position))
    df.dropna(inplace=True)
    
    df['Codons'] = df.apply(lambda df: codons(df, ignore), axis=1)
    df['Phreds'] = df.apply(lambda df: phreds(df), axis=1)
...
def findNear(lead, read, position):
...
def codons(df, ignore):
    ...
    if codon == '':
        ...
        return None
    elif not ignore['Reference Codon'].empty and len(ignore['Reference Codon'].values[0]) != 0 and codon in ignore['Reference Codon'].values[0]:
        return None
    elif not ignore['Reference Codon'].empty and len(ignore['Reference Codon'].values[0]) != 0:
...
def aminoacids_frequencies(df):
    ...
    df = df.explode()
    df.dropna(inplace=True)
    df = pd.DataFrame(df.tolist(), columns=['Genes', 'Codons', 'Position', 'Read', 'Counts'])
    df.dropna(inplace=True)
    codon = df['Codons'].str.split('/', expand=True)
    codon.columns = ['Mutated Codon', 'Reference Codon']
    df = pd.concat([df, codon], axis=1)
    df['Frequencies'] = df['Counts'] * 100 / df['Counts'].sum()
    df['Reference Aminoacid'] = df['Reference Codon'].apply(lambda codon: f'{Seq(codon[:3]).translate()}')
    df['Mutated Aminoacid'] = df['Mutated Codon'].apply(lambda codon: f'{Seq(codon[:3]).translate()}')
    df.drop(columns=['Codons'], inplace=True)
    return df
